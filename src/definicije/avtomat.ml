...
let prazen_avtomat zacetno_stanje =
  {
    ...
    (* zacetni_sklad = Sklad.prazen_sklad; *)
  }
...
let prehodna_funkcija avtomat stanje znak sklad =
  match
    List.find_opt
      (fun (stanje1, znak', sklad1, _stanje2, _sklad2) -> 
        stanje1 = stanje && znak = znak' && sklad1 = sklad)
      avtomat.prehodi
  with
  | None -> None
...
(* let preberi_niz avtomat q niz =
  let aux acc znak =
    match acc with 
    |None -> None 
    |Some q -> Some (prehodna_funkcija avtomat q znak)
  in
